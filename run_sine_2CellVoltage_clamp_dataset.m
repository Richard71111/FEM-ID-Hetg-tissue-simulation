%% RUN_2CELLVOLTAGE_CLAMP_SINE_DATASET
% Slurm-array version — random sine-wave voltage-clamp dataset.
...
%% Protocol settings
dt            = 0.01;   % ms — matches base_cfg.dt
T             = 50;     % ms
Vrest         = -0.879989146999539e2;  % mV
ramp_time     = 2;      % ms, cosine envelope to avoid t=0 current spike
center_range  = [-60, 30];   % mV
amp_range     = [5, 20];     % mV
period_range  = [0.5, 10];   % ms, sampled log-uniformly
command_time  = (0:dt:T)';

base_cfg.T = T;

%% Save folder
save_folder_path = "/fs/ess/PAS1622/RichardSui/FML_ID_dataset/voltage_clamp_sine_ds";
...
for local_case_idx = 1:cases_per_task
    ...
    center    = sample_linear_uniform(center_range, 2);
    amplitude = sample_linear_uniform(amp_range, 2);
    period    = sample_log_uniform(period_range, 2);
    phase     = 2 * pi * rand(2, 1);

    % Per-cell sine wave with cosine ramp envelope from Vrest
    t_col = command_time(:).';                                   % 1-by-Nt
    sine_voltage = center + amplitude .* sin(2*pi*t_col./period + phase);  % 2-by-Nt
    ramp_fraction = min(t_col / ramp_time, 1);
    envelope      = 0.5 * (1 - cos(pi * ramp_fraction));        % 1-by-Nt
    command_voltage = Vrest + envelope .* (sine_voltage - Vrest);

    file_name       = generate_case_filename(base_cfg, case_id);
    local_save_path = fullfile(local_dir, file_name);
    final_save_path = fullfile(target_folder, file_name);

    fprintf("[%s] START task=%d local_case=%d/%d case=%d | center=[%.1f,%.1f] amp=[%.1f,%.1f] period=[%.3f,%.3f]\n", ...
        string(datetime("now", "Format", "yyyy-MM-dd HH:mm:ss")), ...
        task_id, local_case_idx, cases_per_task, case_id, ...
        center(1), center(2), amplitude(1), amplitude(2), period(1), period(2));
    ...
    try
        ...
        save_data(local_save_path, ...
            ...
            vclamp_command, protocol_metadata, ...
            case_id, task_id, local_case_idx);
        ...
        clear result t phi_axial Icleft S_cleft G_state vclamp_command ...
            protocol_metadata center amplitude period phase sine_voltage ...
            ramp_fraction envelope command_voltage
    catch ME
...
values = range(1) + (range(2) - range(1)) * rand(n, 1);
end

function values = sample_log_uniform(range, n)
values = exp(log(range(1)) + (log(range(2)) - log(range(1))) * rand(n, 1));
end

function filename = generate_case_filename(cfg, case_id)
mesh_name = get_mesh_name(cfg.mesh_files);
filename  = sprintf("%s_sine_case_%06d.mat", char(mesh_name), case_id);
end
