function [G, Iion, Ivec, dX, Iall] = fun_ORd11(t,X,p,S)
%FUN_ORD11 Advance the O'Hara-Rudy 2011 ionic model by one time step.
% Inputs: time t, membrane voltage and states X, parameters p, and
% extracellular Na/K/Ca concentrations S.
% Outputs: updated states, total current, species currents, derivatives,
% and individual current components.
% Units:
%
% time in ms
% voltage in mV
...
INa=p.f_I(p.iina,:)'.*GNa.*(v-ENa).*m.^3.*((1.0-fINap).*h.*j+fINap.*hp.*jp);
...
INaL=p.f_I(p.iinal,:)'.*GNaL.*(v-ENa).*mL.*((1.0-fINaLp).*hL+fINaLp.*hLp);
...
ip=AiF.*iFp+AiS.*iSp;
Gto=0.02*ones(Nm,1);
Gto(celltype==1) = 0.02*4;
Gto(celltype==2) = 0.02*4;
% if celltype==1
%     Gto=Gto*4.0;
% elseif celltype==2
%     Gto=Gto*4.0;
...
Ito=p.f_I(p.iito,:)'.*Gto.*(v-EK).*((1.0-fItop).*a.*i+fItop.*ap.*ip);
...
GKr=0.046*ones(Nm,1);
GKr(celltype==1) = 0.046*1.3;
GKr(celltype==2) = 0.046*0.8;
% if celltype==1
%     GKr=GKr*1.3;
% elseif celltype==2
%     GKr=GKr*0.8;
% end
IKr=p.f_I(p.iikr,:)'.*GKr.*sqrt(ko/5.4).*xr.*rkr.*(v-EK);
...
%     GKs=GKs*1.4;
% end
IKs=p.f_I(p.iiks,:)'.*GKs.*KsCa.*xs1.*xs2.*(v-EKs);
...
%     GK1=GK1*1.3;
% end
IK1=p.f_I(p.iik1,:)'.*GK1.*sqrt(ko).*rk1.*xk1.*(v-EK);
...
Gncx(celltype==2) = 0.0008*1.4;
% if celltype==1
%     Gncx=Gncx*1.1;
% elseif celltype==2
%     Gncx=Gncx*1.4;
% end
INaCa_i=p.f_I(p.iinaca_i,:)'.*0.8.*Gncx.*allo.*(zna.*JncxNa+zca.*JncxCa);
...
JncxNa=3.0*(E4.*k7-E1.*k8)+E3.*k4pp-E2.*k3pp;
JncxCa=E2.*k2-E1.*k1;
INaCa_ss=p.f_I(p.iinaca_ss,:)'.*0.2.*Gncx.*allo.*(zna.*JncxNa+zca.*JncxCa);
...
Pnak=30*ones(Nm,1);
Pnak(celltype==1) = 30*0.9;
Pnak(celltype==2) = 30*0.7;
% if celltype==1
%     Pnak=Pnak*0.9;
% elseif celltype==2
%     Pnak=Pnak*0.7;
% end
INaK=p.f_I(p.iinak,:)'.*Pnak.*(zna.*JnakNa+zk.*JnakK);
...
%     GKb=GKb*0.6;
% end
IKb=p.f_I(p.iikb,:)'.*GKb.*xkb.*(v-EK);

%calculate INab
PNab=3.75e-10;
INab=p.f_I(p.iinab,:)'.*PNab.*vffrt.*(nai.*exp(vfrt)-nao)./(exp(vfrt)-1.0);

%calculate ICab
PCab=2.5e-8;
ICab=p.f_I(p.iicab,:)'.*PCab.*4.0.*vffrt.*(cai.*exp(2.0*vfrt)-0.341*cao)./(exp(2.0*vfrt)-1.0);

%calculate IpCa
GpCa=0.0005;
IpCa=p.f_I(p.iipca,:)'.*GpCa.*cai./(0.0005+cai);
...
Iion=Rcg*p.Ctot.*(INa+INaL+Ito+ICaL+ICaNa+ICaK+IKr+IKs+IK1+INaCa_i+INaCa_ss+INaK+INab+IKb+IpCa+ICab-Istim);
...
Ivec(1:Nm) = Rcg*p.Ctot.*(INa+INaL+3*INaCa_i+3*INaK+INab + 3*INaCa_ss+ICaNa);      % Na+
Ivec(Nm+1:2*Nm) = Rcg*p.Ctot.*(Ito+IKr+IKs+IK1+IKb-Istim-2*INaK + ICaK); % K+
Ivec(2*Nm+1:3*Nm) = Rcg*p.Ctot.*(IpCa+ICab-2*INaCa_i + ICaL-2*INaCa_ss);      % Ca2+

% all ionic currents
[Ncurrents,~] = size(p.f_I);
Iall = nan(Nm*Ncurrents,1);

Iall(p.iina:Ncurrents:end) = Rcg*p.Ctot.*INa;
Iall(p.iinal:Ncurrents:end) = Rcg*p.Ctot.*INaL;
Iall(p.iito:Ncurrents:end) = Rcg*p.Ctot.*Ito;
Iall(p.iical:Ncurrents:end) = Rcg*p.Ctot.*ICaL;
Iall(p.iikr:Ncurrents:end) = Rcg*p.Ctot.*IKr;
Iall(p.iiks:Ncurrents:end) = Rcg*p.Ctot.*IKs;
Iall(p.iik1:Ncurrents:end) = Rcg*p.Ctot.*IK1;
Iall(p.iinaca_i:Ncurrents:end) = Rcg*p.Ctot.*INaCa_i;
Iall(p.iinaca_ss:Ncurrents:end) = Rcg*p.Ctot.*INaCa_ss;
Iall(p.iinak:Ncurrents:end) = Rcg*p.Ctot.*INaK;
Iall(p.iikb:Ncurrents:end) = Rcg*p.Ctot.*IKb;
Iall(p.iinab:Ncurrents:end) = Rcg*p.Ctot.*INab;
Iall(p.iicab:Ncurrents:end) = Rcg*p.Ctot.*ICab;
Iall(p.iipca:Ncurrents:end) = Rcg*p.Ctot.*IpCa;
...
JintraNai = 0; %sum((nai(p.ind_tau_ip) - nai(p.ind_tau_im))./p.tau_Nai_mat,2);
JintraCai = 0; % sum((cai(p.ind_tau_ip) - cai(p.ind_tau_im))./p.tau_Cai_mat,2);
JintraKi = 0; % sum((ki(p.ind_tau_ip) - ki(p.ind_tau_im))./p.tau_Ki_mat,2);
