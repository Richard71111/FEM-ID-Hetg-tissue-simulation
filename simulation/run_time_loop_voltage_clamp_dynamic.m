function result = run_time_loop_voltage_clamp_dynamic(cfg, topology, model, network)
%RUN_TIME_LOOP_VOLTAGE_CLAMP_DYNAMIC Advance with a sampled clamp waveform.
...
patch = network.patch;
mesh = network.mesh;
protocol = normalize_dynamic_vclamp_protocol(cfg.vclamp, Ncell, cfg.T);
all_nodes = (1:Nnodes)';
...
function protocol = normalize_dynamic_vclamp_protocol(vclamp, Ncell, T)
%NORMALIZE_DYNAMIC_VCLAMP_PROTOCOL Validate a sampled voltage waveform.
required = ["command_time", "command_voltage"];
for name = required
    if ~isfield(vclamp, name)
        error("cfg.vclamp.%s is required for dynamic clamp.", name);
    end
end
...
command_time = double(vclamp.command_time(:).');
command_voltage = double(vclamp.command_voltage);
if size(command_voltage, 1) ~= numel(cells) || ...
        size(command_voltage, 2) ~= numel(command_time)
    error("command_voltage must have shape [numel(cells), numel(command_time)].");
end
if numel(command_time) < 2 || any(diff(command_time) <= 0)
    error("command_time must be a strictly increasing vector.");
end
if command_time(1) > 0 || command_time(end) < T
    error("command_time must cover the full simulation interval [0,T].");
end
if any(~isfinite(command_voltage), "all")
    error("command_voltage contains NaN or Inf.");
end

protocol = struct();
protocol.mode = "waveform";
protocol.cells = cells;
protocol.command_time = command_time;
protocol.command_voltage = command_voltage;
protocol.interpolation = "linear";
if isfield(vclamp, "interpolation")
    protocol.interpolation = string(vclamp.interpolation);
end
protocol.events = T;
end

function [active_cells, command] = voltage_clamp_command(t, protocol)
%VOLTAGE_CLAMP_COMMAND Interpolate all actively clamped cell commands.
active_cells = protocol.cells;
t_query = min(max(t, protocol.command_time(1)), protocol.command_time(end));
command = interp1( ...
    protocol.command_time, protocol.command_voltage.', t_query, ...
